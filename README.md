...
### 1.2 修复内容
- AI 资产顾问
  - 修复自建公益站模型输入体验，支持前端自由填写模型名并记住上次选择。
  - 优化 AI 错误处理与提示：模型不存在、渠道不可用、上游 503、超时等场景给出明确反馈。
  - 优化分析 Prompt 与 Markdown 渲染稳定性，减少表格/标题错乱与超时概率。
- Telegram Bot
  - 修复 `/help` 在 HTML parse mode 下因 `<...>` 占位符导致的指令无响应问题。
  - 新增 `/detail <ID/关键词>` 指令，并接入命令分发、帮助文案、Telegram 命令菜单。
  - AI 回答不再推荐未实现命令，提升可用性一致性。
  - 优化 TG AI 调用链：压缩上下文、模型回退、错误文案细化。
- 通知配置
  - 修复“邮件通知配置刷新后被重置”的问题，保留本地已配置的 Email/Telegram/Webhook 设置并安全回写服务端。
