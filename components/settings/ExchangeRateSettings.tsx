...
const ExchangeRateSettings: React.FC = () => {
  ...
  return (
    <div className="bg-white/90 dark:bg-slate-900/70 border border-white/60 dark:border-slate-800/60 rounded-2xl p-6 shadow-sm animate-fade-in">
      ...
      <div className="flex flex-wrap items-center gap-3 mt-6 relative z-10 pointer-events-auto">
        <button
          type="button"
          onClick={handleSave}
        ...
        <button
          ...
          onClick={loadRates}
          disabled={loading}
          className="px-4 py-2.5 bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-200 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors text-sm font-semibold shadow-sm flex items-center gap-2 disabled:opacity-60 disabled:cursor-not-allowed"
        >
