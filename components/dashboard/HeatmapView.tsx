...
const HeatmapView: React.FC<HeatmapViewProps> = ({ resources }) => {
  ...
  const getDotColor = (type: ResourceType) => {
    switch (type) {
      ...
      case ResourceType.DOMAIN: return 'bg-indigo-400';
      case ResourceType.PHONE_NUMBER: return 'bg-sky-400';
  ...
  return (
    <div className="bg-white/90 dark:bg-slate-900/70 p-8 rounded-3xl border border-white/60 dark:border-slate-800/60 shadow-sm animate-fade-in overflow-x-auto">
      ...
      <div className="min-w-max">
            ...
            return (
              <div
                ...
                className={`w-3.5 h-3.5 rounded-sm transition-all duration-200 relative group
                  ${hasExpiry
                    ? (isUrgent ? 'bg-rose-500 shadow-sm shadow-rose-200/50' : 'bg-indigo-500')
        ...
        <div className="mt-4 flex items-center gap-4 text-xs text-slate-400 font-medium">
          ...
          <div className="flex items-center gap-1"><div className="w-3 h-3 bg-rose-500 rounded-sm"></div> 30 天内紧急</div>
        </div>
      </div>
    </div>
  );
